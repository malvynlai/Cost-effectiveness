...
#  1. COMPLETE RUNS
#  2. PRECOMPUTE PROBABILITIES
#  3. RANDOM NUMBER GENERATION
#  4. TRANSITION MATRICES WITH AGE
#  5. UTILITY FUNCTIONS AND TESTS
#  6. IMPORTANT UTILITY FUNCTIONS

#  Overall flow: call complete run
#  1. Generate data with utility/obtain transition matrix, use it to precompute probabilities, obtain cumulative
#  2. THen use random number generation and cumulative to obtain matrix of states.
#  3. Real-world data not obtained in this file, refer to compressedInputsRead.py
...
# You run this to return states. Function also caculates utilities, costs but these are not returned
# triple is (discountRate, num_steps, num_chains)
def realLifeSimulation(transition_matrix, rewardVector, utilityVector, *triple):
   ...
   # BASE PARAMETERS
    ...
    # RANDOM_NUMBERS
    states, hcc_costs = simulate_markov_chain_with_cache(transition_matrix, cumulative_probs_cache, initial_state, num_steps, num_chains, stop_state)
    ...
    return oldStates2, utilities, rewards

#transition matrix is 7 x 581
#cumulative prob cache should also be 7 x 581
def simulationwithAge(transition_matrix, rewardVector, utilityVector, ageVector, random_numbers, discountRate, num_steps, num_chains):
    if not check_same_size(rewardVector, utilityVector):
        print("Input size error!")
        sys.exit()
    ...
    states, hcc_costs = simulate_with_cache_ageDistribution(cumulative_probs_cache, ageVector, rewardVector, discountRate, random_numbers, *quad)
    oldStates = copy.deepcopy(states)
    oldStates2 = copy.deepcopy(oldStates)
    ...
    rewards = populate_matrix_with_indices(rewardVector, states)
    rewards += hcc_costs
    rewards2 = rewards * discountMatrix.T
    rewards = rewards @ discountMatrix
    ...
    integerUtility = 100*utilityVector  #  as dtype of states is int64
    utilities =  populate_matrix_with_indices(integerUtility, oldStates)
    utilities2 = utilities * discountMatrix.T
    utilities2 /= 100
    utilities = utilities @ discountMatrix
    utilities = utilities/100 
    return oldStates2, utilities, utilities2, rewards, rewards2
...
# Takes a num_states x num_states matrix and outputs cumulative matrix
# Transition matrix rows must sum to 1
# Cumulative matrix rows must be strictly increasing and last state should be 1
@njit
...
# Takes a num_states x (num_states*num_ages) matrix and returns object of same shape
# Each num_states x num_states block corresponds for a transition matrix for that age
# Example: states = 10, ages = 8 then the block for age 6 is [0:10,10*6:10*7]. 
# Note ages from agevector start at 0-82 but actually represent ages 18-100 
...
# Takes in cumulative matrix, initial start state for all patients, 
# num_steps: i.e number of years for patients in simulation. Range : 1-100. All should be dead after 100.
# num_chains: num of disctinct patients
# stop_state: the death state
# Returns Matrix states of size num_chains x num_steps, state for each patient at time t
# Note that having s[i,j] = x means patient i is in state x at time j. Integer x can be mapped to state by using transition matrix column order (i.e State 0 is MALSD, etc...)
@njit(parallel=True)
def simulate_markov_chain_with_cache(transition_matrix, cumulative_probs_cache, initial_state, num_steps, num_chains, stop_state, random_numbers):
    num_states = transition_matrix.shape[0]
    states = np.zeros((num_chains, num_steps), dtype=np.int64)
    states[:, 0] = initial_state

    # Use provided random numbers
...
# Same as above  but with agelogic
# ageVector is num_chains x 1
# Difference with above is that we need to find age of each patient at each step, and then select the right slice from 
# Cumulative matrix to match the age of that patient. Also implements checks that slice is valid cumulative construct
@njit
def simulate_with_cache_ageDistribution(cumulative_probs_cache, ageVector, rewardVector, discountRate, random_numbers, *quad):
    np.random.seed(42)
    initial_state, num_steps, num_chains, stop_state = quad
    num_states = cumulative_probs_cache.shape[0]
    states = np.zeros((num_chains, num_steps), dtype=np.int64)
    states[:, 0] = initial_state
    hcc_costs = np.zeros_like(states)
    ...
    for t in range(1, num_steps):
        for i in range(num_chains):
            ...
            if states[i, t] == 1:  
                cumulative_probs = cumulative_probs_cache[1, translationIndex: translationIndex + 7] # 1 for HCC state, add 7 for total num states
                states[i, t] = np.searchsorted(cumulative_probs, np.random.rand())
                hcc_costs[i, t] = 1/(1 + discountRate) ** t * rewardVector[1, 0] # Add discounted hcc cost
                
    return states, hcc_costs
...
def generate_transition_matrixAgeDistribution(transition_matrix, size, probDeath, posDeath):
    ...
    for x in range(0, num_ages):
            matrix[0,posDeath + x*size] = probDeath[x] #transition_matrix[0,posDeath] + probDeath[x]    # increment death node
...
# Use this to test functions on dummy data, can be used to generate dummy data
...
#########   IMPORTANT UTILTIY FUNCTIONS ##########

# Populates matrix of states with values to replace the states
# The state matrix is populated by states 0,1,2,3,4
# Now say we havecost values associated to each state so we have 100,-200,3,5,140 for each state
# This function replaces the state indices by state values according to v, returning a state-value matrix
# Very useful to compute costs, utility values for a simulation once we get states
@njit(parallel=True)
def populate_matrix_with_indices(v, matrix):
    shape = matrix.shape
    oldMatrix = matrix #as we change value of matrix
    
    for i in prange(shape[0]):  # Iterate over rows
...
# Testing a full run with dummy data with age distribution.
def testRunAge():
    ...
    c = MC.precompute_cumulative_probabilitieswithAge(transition_matrix)
...
# Makes sure all the dimensions are correct for age distribution simulation run
...
# gere we actually fix the state matrix to check calculations Ffor utilities
